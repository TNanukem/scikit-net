...
class KNNConstructor(BaseConstructor):
    ...
    def fit(self, X, y=None):
        ...
        the node information under the label 'class'
        ...
        neighbors = [tree.query(x.reshape(1, -1), k=self.k+1, return_distance=True) for x in X]  # noqa: E501
        ...
        edges = [(node[0], node[j], distances[i][j]) for i, node in enumerate(indexes) for j in range(1, self.k+1)]  # noqa: E501
...
class EpsilonRadiusConstructor(BaseConstructor):
    ...
    def fit(self, X, y=None):
        ...
        neighbors = [tree.query_radius(x.reshape(1, -1), r=self.epsilon, return_distance=True, sort_results=True) for x in X]  # noqa: E501
        ...
        edges = [(node[0], node[j], distances[i][j]) for i, node in enumerate(indexes) for j in range(1, len(node))]  # noqa: E501
...
class KNNEpislonRadiusConstructor(BaseConstructor):
    ...
    def fit(self, X, y=None):
        ...
        Notes
        -----
        If y is set, then the class of each node will be inserted into the
        node information under the label 'class'
        """
        ...
        radius_neighbors = [tree.query_radius(x.reshape(1, -1), r=self.epsilon, return_distance=True, sort_results=True) for x in X]  # noqa: E501
        k_neighbors = [tree.query(x.reshape(1, -1), k=self.k+1, return_distance=True) for x in X]  # noqa: E501
        ...
        edges_radius = [(node[0], node[j], final_radius_distances[i][j]) for i, node in enumerate(final_radius) for j in range(1, len(node))]  # noqa: E501
        edges_k = [(node[0], node[j], final_k_distances[i][j]) for i, node in enumerate(final_k) for j in range(1, self.k+1)]  # noqa: E501
